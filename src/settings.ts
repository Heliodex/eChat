...
export const historyLength = writable(
	localStorage.getItem("historyLength") || "100"
)
export const transitionLength = writable(
	localStorage.getItem("transitionLength") || "1"
)
export const hideChannel = writable(
	localStorage.getItem("hideChannel") == "true"
)
...
function changeTheme(themeValue: string) {
	gsap.to(":root", {
		duration: 0.3 * parseFloat(get(transitionLength)),
		"--main": themes[themeValue][0],
	})
	gsap.to(":root", {
		duration: 0.3 * parseFloat(get(transitionLength)),
		"--main-dark": themes[themeValue][1],
	})
}
...
const superSecretSettings = [
	...
	[
		() =>
			document.documentElement.style.setProperty(
				"--transform",
				`scale(${Math.random() > 0.5 ? 0.9 : -1}, ${
					Math.random() > 0.5 ? 0.9 : -1
				})`
			),
		() => document.documentElement.style.setProperty("--transform", ""),
	],
